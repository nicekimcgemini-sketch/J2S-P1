...
- **프론트**: 스타일링은 Tailwind CSS v4(`@tailwindcss/vite` 플러그인, `src/index.css` 의 `@theme` 블록에 `brand` 색상/폰트 토큰 정의)로 전면 전환했다. 클래스 기반 CSS 파일(`theme.css`)은 삭제됨 — 새 UI는 전부 Tailwind 유틸리티 클래스로 작성한다. 반복되는 패턴(통계 카드, 상태 배지, 패널, 버튼, 입력 필드)은 `src/components/dashboard.tsx` 의 공용 컴포넌트를 재사용한다. 아이콘은 `lucide-react`. 다크 "가을" 톤(`index.css`에서 기본 `slate` 팔레트를 스톤 계열 따뜻한 색으로 오버라이드 + 호박/단풍색 `brand` 액센트 + 에메랄드/앰버/로즈 상태색)으로 통일했으니 새 화면도 이 톤을 따른다. 제목류(`PageHeader`, 카드 헤드라인)는 `font-display`(Noto Serif KR)를 쓰고 본문/UI 요소는 계속 `font-sans`(Pretendard)로 — 서체 하나로만 위계를 표현하지 않는다. 카드/패널은 반투명+blur("유리" 효과), 큰 라운드(`rounded-xl` 이상), 진한 글로우 섀도우를 쓰지 않는다 — 불투명한 `bg-slate-900` + 1px `border-slate-800` 평면 표면이 기본이고, 단일 지점 강조가 필요하면 `border-t-2 border-t-brand-500` 같은 얇은 액센트 선 하나로 처리한다(로그인/QR/체크인 셸 참고). 아이콘을 파스텔 톤 원형 배경에 얹는 "기능 카드" 패턴, 이모지 헤드라인, 배경 라디얼 그라데이션 글로우는 전형적인 생성형 UI 티가 나므로 새로 추가하지 말 것 — 안내가 필요하면 QrScreen의 번호 매긴 목록처럼 타이포그래피로 해결한다. 상태는 컴포넌트 로컬 `useState`, 전역 상태 라이브러리 없음. 관리자 인증 토큰은 `sessionStorage` 의 `admin_auth`.
